...
from services.notifier.repo import insert_notification_if_absent, get_notification, mark_sent, mark_failed, list_due_failed, backoff_seconds
import datetime
...
async def run_notifier_stream_consumer() -> None:
    ...
    while True:
        # 轮询两个 stream（简化：先 exec_report，再 risk）
        for stream, schema in [(STREAM_EXEC_REPORT, EXEC_REPORT_SCHEMA), (STREAM_RISK, RISK_EVENT_SCHEMA)]:
            msgs = client.read_group(stream, settings.redis_stream_group, settings.redis_stream_consumer, count=20, block_ms=500)
            if not msgs:
                continue
            for m in msgs:
                try:
                    ...
                    # Stage 3：落库（幂等）—— 以 event_id 作为 notification_id
                    ...
                    try:
                        ...
                        # 落库失败不影响消费；后续可通过日志定位 DB 问题
                        pass

                    # Stage 3：幂等发送
                    st = None
                    try:
                        st = get_notification(settings.database_url, notification_id=str(notification_id))
                    except Exception:
                        st = None
                    ...
                    ok = False
                    try:
                        ok = _maybe_notify(text=text, severity=sev)
                    ...
                    if ok:
                        try:
                            mark_sent(settings.database_url, notification_id=str(notification_id))
                        except Exception:
                            pass
                    else:
                        # 失败：记录失败并安排重试
                        try:
                            ...
                            mark_failed(settings.database_url, notification_id=str(notification_id), attempts=attempts, next_attempt_at=nxt, last_error=locals().get("err", "send_failed"))
                        except Exception:
                            pass

                    # 无论成功/失败都 ACK，避免重复消费导致风暴
                    client.ack(m.stream, settings.redis_stream_group, m.message_id)
                except Exception as e:
                    try:
                        publish_dlq(settings.redis_url, source_stream=stream, message_id=m.message_id, reason=str(e), raw_fields=m.fields)
                    except Exception:
                        pass
                    logger.warning("notify_failed", extra={"extra_fields": {"stream": stream, "error": str(e)}})
                    client.ack(m.stream, settings.redis_stream_group, m.message_id)
        await asyncio.sleep(0.1)


async def run_retry_loop() -> None:
    """Stage 3：失败通知重试循环（DB 驱动）。"""
    while True:
        try:
            due = list_due_failed(settings.database_url, max_attempts=int(settings.notifier_max_attempts), limit=20)
            for row in due:
